...
assertTaskMetadata(
    $parsed['summary'] === 'Versicherung prüfen'
        ...
        && $parsed['status'] === 'TENTATIVE'
        && $parsed['transparency'] === 'TRANSPARENT',
    'iCalendar task metadata must round-trip without replacing the independent 9.1 event state.'
);
...
$normalIcal = ICalendarCodec::updateEvent($updatedIcal, $created['uid'], [
    'summary' => 'Versicherung prüfen', 'task' => false
]);
$normalParsed = ICalendarCodec::parseEvents($normalIcal, 'https://calendar.example/task.ics', 'etag')[0];
assertTaskMetadata(
    !str_contains($normalIcal, 'X-OPENCALENDAR-TASK')
        && !str_contains($normalIcal, 'X-OPENCALENDAR-ROLL-FORWARD')
        && !isset($normalParsed['task'])
        && $normalParsed['status'] === 'TENTATIVE'
        && $normalParsed['transparency'] === 'TRANSPARENT',
    'Converting an iCalendar task must clear only task metadata and retain the 9.1 event state.'
);
...
assertTaskMetadata(
    $googlePayload['summary'] === 'Versicherung prüfen'
        ...
        && $googlePayload['status'] === 'tentative'
        && $googlePayload['transparency'] === 'transparent',
    'Google task metadata must keep the visible title clean and preserve the 9.1 event state.'
);

$googleNormalClient = new TaskMetadataHttpClient([
    new CalendarHttpResponse(200, [], json_encode($googleResponse, JSON_THROW_ON_ERROR), 'https://www.googleapis.com/calendar/v3/calendars/cal/events')
]);
$googleNormal = new GoogleCalendarProvider($googleNormalClient, 'token');
$googleNormal->createEvent('cal', [
    'summary' => 'Normal appointment', 'allDay' => true,
    'start'   => '2026-09-20', 'end' => '2026-09-21', 'task' => false
]);
$googleNormalPayload = json_decode($googleNormalClient->requests[0]['body'], true, 512, JSON_THROW_ON_ERROR);
assertTaskMetadata(
    !array_key_exists('extendedProperties', $googleNormalPayload),
    'Creating an ordinary Google event must omit null task properties that cause a Required API error.'
);
