...
Google-Serienvorkommnisse können einzeln, ab dem ausgewählten Vorkommnis oder als vollständige Serie bearbeitet und gelöscht werden. Beim **Bearbeiten** teilt OpenCalendar die bestehende Google-Serie am gewählten Vorkommnis: Der vordere Serienteil bleibt unverändert, ab dem gewählten Termin entsteht ein neuer Serienteil mit den Änderungen. Bereits vorhandene Ausnahmen ab diesem Termin werden dabei zurückgesetzt. Beim **Löschen** beendet OpenCalendar die bestehende Serie direkt vor dem gewählten Vorkommnis; beim ersten Vorkommnis wird die komplette Serie entfernt. Bei Microsoft können einzelne Vorkommnisse und die vollständige Serie bearbeitet und gelöscht werden; **Diesen und alle folgenden Termine** bleibt vorerst geschützt. CalDAV-Serienvorkommnisse werden weiterhin nur lesend dargestellt.
...
Ein Klick auf einen Termin öffnet zunächst eine reine Termindetail-Ansicht mit Kalender, Beginn, Ende, Ort und Beschreibung. Bei Google-Serienvorkommnissen stehen beim Bearbeiten und Löschen **Nur diesen Termin**, **Diesen und alle folgenden Termine** und **Gesamte Serie** zur Wahl. Für die gesamte Serie lädt OpenCalendar zuerst den Parent-Termin. Bei „Diesen und alle folgenden Termine“ werden zusätzlich das konkrete Zielvorkommnis und die verbleibende Serienlänge verifiziert, bevor der Editor geöffnet wird. Diese Teilung wird nur für den von OpenCalendar verlustfrei unterstützten einfachen RRULE-Umfang angeboten; komplexere Regeln werden nicht automatisch geteilt. Bei Microsoft stehen **Nur diesen Termin** und **Gesamte Serie** zur Verfügung. Für die vollständige Serie lädt OpenCalendar ebenfalls den Serien-Master; unterstützte Microsoft-Muster können im gemeinsamen Serieneditor geändert werden. **Diesen und alle folgenden Termine** bleibt bei Microsoft noch geschützt. CalDAV-Vorkommnisse sowie schreibgeschützte Kalender bleiben geschützt.

Beim Erstellen eines Termins bieten beschreibbare Google- und Microsoft-Kalender zusätzlich den Bereich **Wiederholen**. Unterstützt werden tägliche, wöchentliche, monatliche und jährliche Serien, ein frei wählbares Intervall, bei wöchentlichen Serien mehrere Wochentage sowie die Endarten **Nie**, **Nach Anzahl** und **Am Datum**. Die Serienoption erscheint nur bei Kalendern, deren Provider das Anlegen von Serienterminen ausdrücklich unterstützt. Für zeitgebundene Google-Serien wird die Kalenderzeitzone verwendet; bei Microsoft dient die Zeitzone des Clients als Rückfallwert, wenn der Kalender selbst keine Zeitzone bereitstellt.
