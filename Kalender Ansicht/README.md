...
In der Monatsansicht ist der Hinweis `+ weitere` anklickbar. Er öffnet eine Tagesübersicht mit allen Terminen dieses Tages. Die Termine werden ganztägig zuerst und anschließend chronologisch dargestellt; zusätzlich zeigt die Übersicht die Terminanzahl. Ein Klick auf einen Termin öffnet zunächst die Termindetail-Ansicht. Von dort kann ein schreibbarer Einzeltermin gezielt bearbeitet werden. Ist mindestens ein beschreibbarer Kalender verfügbar, kann über **Termin an diesem Tag erstellen** direkt ein neuer Termin für den ausgewählten Tag angelegt werden.
