...
Agenda, 3-Tage-, Wochen- und Monatsansicht sowie die Navigation funktionieren direkt innerhalb der IPSView-HTMLBox. In beschreibbaren Kalendern können dort außerdem Termine erstellt, bearbeitet und gelöscht sowie alle ausgewählten Kalender manuell synchronisiert werden. Wiederkehrende Einzelvorkommen bleiben weiterhin schreibgeschützt. Die Variable wird bei Änderungen oder Synchronisationen der ausgewählten Kalender automatisch neu erzeugt. Wird die IPSView-Ausgabe deaktiviert, bleibt eine bereits vorhandene Variable mit ihrer Objekt-ID, Positionierung und bestehenden Verknüpfungen erhalten und wird nicht mehr aktualisiert. In der Instanzkonfiguration erscheint dann eine gesonderte Löschaktion; erst nach ausdrücklicher Bestätigung wird die Variable entfernt.

Da IPSView keine Symcon-HTML-SDK-Funktion `requestAction()` bereitstellt, verwendet die IPSView-Seite eine instanzbezogene WebHook-Brücke. Das Modul erzeugt dafür ein zufälliges, persistent gespeichertes Zugriffstoken und akzeptiert ausschließlich `GetState`, `Refresh`, `CreateEvent`, `UpdateEvent` und `DeleteEvent` per POST. Das Token ist Bestandteil der erzeugten WebContent-Seite und sollte wie die IPSView-/Symcon-Verbindung selbst geschützt werden. Für einen Zugriff außerhalb des eigenen Netzes sollte ausschließlich eine verschlüsselte HTTPS- oder Symcon-Connect-Verbindung verwendet werden.
